...
export const Form = <TFormValues extends FieldValues>(props: Props<TFormValues>) => {
  const { onSubmit, defaultValues, validationSchema, children, className } = props
  ...
  useEffect(() => {
    isSubmitSuccessful && !errors.root?.serverError && reset()
  }, [isSubmitSuccessful, reset, errors])

  return (
    <form onSubmit={methods.handleSubmit(onSubmitHandle)} className={classNames(className)}>
